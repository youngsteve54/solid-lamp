...
// -----------------------
// TELEGRAM BOT INSTANCE (set later)
// -----------------------
...
async function notifyUser(userId, text, imgBuffer = null) {
  if (!telegramBotInstance) return console.warn("Telegram bot not initialized yet.");
  try {
    if (imgBuffer) {
      await telegramBotInstance.sendPhoto(userId, imgBuffer, { caption: text, parse_mode: "Markdown" });
    } else {
      await telegramBotInstance.sendMessage(userId, text, { parse_mode: "Markdown" });
    }
  } catch (err) {
...
// -----------------------
export function bindTelegramEvents(botInstance) {
  setTelegramBot(botInstance);

  botInstance.on("link_whatsapp", async ({ userId, number, method }) => {
    await WhatsAppManager.startSession(userId, number, method);
  });

  botInstance.on("unlink_whatsapp", async ({ userId, number }) => {
